...
@dp.message_handler(text="Stylesl")
async def reply_button_handler(message: types.Message):
    await message.answer("Styles", callback_data='styles')

@dp.message_handler()
async def styles(message: types.Message):
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton('Preppy', callback_data='preppy_style'))
    markup.add(types.InlineKeyboardButton('Casual', callback_data='casual_style'))
    await message.reply('Hello', reply_markup=markup)
